  ...
  setupGameLoop() {
    setInterval(() => {
      if (this.state.status === "playing") {
        ...
        // Spawn gems in Gem Grab mode
        if (this.party.id.includes("gem_grab") && this.state.gems.length < 10 && Math.random() < 0.1) {
           const newGem = { id: Math.random().toString(), x: (Math.random() - 0.5) * 10, z: (Math.random() - 0.5) * 10 };
        ...
        if (this.state.timer <= 0) {
          ...
          this.party.broadcast(JSON.stringify({ type: "gameOver", state: this.state }));
        }
      }
    }, 1000);
  }
  ...
  onMessage(message: string, sender: PartyConnection) {
    ...
    if (data.type === "update") {
      if (this.state.players[sender.id] && !this.state.players[sender.id].isDead) {
       ...
       if (isSafe && this.party.id.includes("heist")) {
         ...
         if (victim.health <= 0) {
           ...
           setTimeout(() => {
             if (this.state.players[victimId]) {
               ...
               this.state.players[victimId].x = this.state.players[victimId].team === 0 ? -20 : 20;
