...
export default function GameScene({ brawlerId, mode }: GameSceneProps) {
  ...
  useEffect(() => {
    ...
    const camera = new BABYLON.FreeCamera("camera1", new BABYLON.Vector3(0, 45, -35), scene);
    ...
    const createBrawlerMesh = (id: string, configId: string, isLocal: boolean, team: number) => {
      ...
      let body;
      if (config.health > 5000) {
        body = BABYLON.MeshBuilder.CreateBox("body-" + id, { width: 1.8, height: 2.2, depth: 1.8 }, scene);
      } else if (config.attackRange > 9) {
        body = BABYLON.MeshBuilder.CreateCylinder("body-" + id, { diameter: 1.2, height: 2 }, scene);
      } else {
        body = BABYLON.MeshBuilder.CreateBox("body-" + id, { width: 1.2, height: 1.8, depth: 1.2 }, scene);
      }
      body.position.y = body.scaling.y * 0.9;
      ...
      body.material = mat;
      if (isLocal) {
        const indicator = BABYLON.MeshBuilder.CreateDisc("indicator", { radius: 1.8 }, scene);
    ...
    const mapCells = mode === "random" ? generateRandomMap(60, 60) : MAPS.default.cells;
    mapCells.forEach(cell => {
      if (cell.type === "wall") {
        ...
        mat.diffuseColor = new BABYLON.Color3(0.4, 0.4, 0.4); wall.material = mat;
        walls.push(wall);
        ...
        mat.diffuseColor = new BABYLON.Color3(0.1, 0.4, 0.1); mat.alpha = 0.6; bush.material = mat;
        bushes.push(bush);
      }
    });

    if (mode === "heist") {
      const s0 = BABYLON.MeshBuilder.CreateBox("safe-0", { size: 4 }, scene);
      s0.position.set(-30, 2, 0);
      const m0 = new BABYLON.StandardMaterial("m0", scene); m0.diffuseColor = BABYLON.Color3.Blue(); s0.material = m0;
      safes[0] = s0;
      const s1 = BABYLON.MeshBuilder.CreateBox("safe-1", { size: 4 }, scene);
      s1.position.set(30, 2, 0);
    ...
    const shoot = (isSuper = false) => {
      ...
      // Shotgun spread for some brawlers
      ...
      for (let i = 0; i < count; i++) {
        ...
        socket.send(JSON.stringify({
          ...
          projectile: {
            ...
            ownerId: socket.id, team: myTeam, x: player.container.position.x, z: player.container.position.z,
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data.type === "init") {
        ...
        (player.body.material as BABYLON.StandardMaterial).diffuseColor = BABYLON.Color3.FromHexString(myTeam === 0 ? "#3b82f6" : "#ef4444");
        ...
        Object.keys(data.state.players).forEach(id => {
          ...
          otherPlayers[id].body.setEnabled(!pD.isDead);
          otherPlayers[id].body.visibility = (pD.isInBush && myTeam !== pD.team) ? 0 : 1;
        });
        setHudData(prev => ({ ...prev, timeLeft: data.state.timer }));
      } else if (data.type === "shoot") {
        const pD = data.projectile;
        const pM = BABYLON.MeshBuilder.CreateSphere("p", { diameter: pD.isSuper ? 1.0 : 0.6 }, scene);
        pM.position.set(pD.x, 1.2, pD.z);
        const pMat = new BABYLON.StandardMaterial("pm", scene);
        pMat.emissiveColor = pD.team === myTeam ? new BABYLON.Color3(0.5, 0.5, 1) : new BABYLON.Color3(1, 0.5, 0.5);
        pM.material = pMat;
        let tr = 0;
        const obs = scene.onBeforeRenderObservable.add(() => {
          ...
          if (hitWall) { scene.onBeforeRenderObservable.remove(obs); pM.dispose(); return; }
          if (pD.team !== myTeam && !isDead && BABYLON.Vector3.Distance(pM.position, player.container.position.add(new BABYLON.Vector3(0, 1, 0))) < 1.5) {
            socket.send(JSON.stringify({ type: "hit", victimId: socket.id, damage: pD.damage }));
            scene.onBeforeRenderObservable.remove(obs); pM.dispose();
          } else if (pD.team === myTeam) {
            Object.keys(otherPlayers).forEach(oid => {
              if (otherPlayers[oid].team !== myTeam && BABYLON.Vector3.Distance(pM.position, otherPlayers[oid].container.position.add(new BABYLON.Vector3(0, 1, 0))) < 1.5) {
                superCharge = Math.min(100, superCharge + 12); setHudData(prev => ({ ...prev, superCharge }));
                scene.onBeforeRenderObservable.remove(obs); pM.dispose();
              }
            });
            if (mode === "heist") {
              const enemySafe = safes[1 - myTeam];
              if (enemySafe && BABYLON.Vector3.Distance(pM.position, enemySafe.position) < 3) {
        ...
        if (data.id === socket.id) { isDead = true; player.body.setEnabled(false); setHudData(prev => ({ ...prev, isDead: true })); }
        else if (otherPlayers[data.id]) otherPlayers[data.id].body.setEnabled(false);
      } else if (data.type === "respawn") {
        if (data.id === socket.id) { isDead = false; health = data.state.health; player.container.position.set(data.state.x, 0, data.state.z); player.body.setEnabled(true); setHudData(prev => ({ ...prev, health, isDead: false })); }
        else if (otherPlayers[data.id]) { otherPlayers[data.id].body.setEnabled(true); otherPlayers[data.id].container.position.set(data.state.x, 0, data.state.z); }
    ...
    <div className="relative w-full h-screen bg-slate-900 overflow-hidden font-sans">
      ...
      {hudData.isDead && !hudData.gameOver && (
        <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center z-50">
          <h2 className="text-6xl font-black italic text-red-500 mb-4">DEFEATED</h2>
          <p className="text-white text-xl animate-pulse">Respawning soon...</p>
        </div>
      )}
      <div className="absolute top-6 left-1/2 -translate-x-1/2 flex gap-8 items-center bg-black/40 backdrop-blur-md px-8 py-3 rounded-3xl border-2 border-white/20">
         ...
         <div className="text-white font-black text-2xl">{Math.floor(hudData.timeLeft/60)}:{(hudData.timeLeft%60).toString().padStart(2,'0')}</div>
      </div>
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 w-80">
        ...
        <div className="flex gap-4 w-full items-end">
          ...
          <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all border-4 ${hudData.gadgets > 0 ? "bg-green-600 border-white" : "bg-slate-800 border-slate-700 opacity-50"}`}>
          ...
          <div className={`w-20 h-20 rounded-full border-4 flex flex-col items-center justify-center transition-all ${hudData.superCharge >= 100 ? "bg-yellow-500 border-white scale-110 shadow-[0_0_25px_rgba(234,179,8,1)]" : "bg-slate-800 border-slate-600 scale-90"}`}>
      ...
      <div className="absolute top-6 left-6 bg-black/40 backdrop-blur-md px-4 py-2 rounded-2xl border border-white/20">
        <div className="text-2xl font-black italic text-yellow-400 tracking-wider uppercase">{mode}</div>
        <div className="text-sm font-bold text-white/80 uppercase">{BRAWLERS[brawlerId].name}</div>
      </div>
    </div>
  );
}
